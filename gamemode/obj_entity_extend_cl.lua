...
GM.CachedSpawns = {}
timer.Create("CachedSpawns", 0.5, 0, function()
	if not GAMEMODE then return end
	GAMEMODE.CachedSpawns = {}

	local spawns = ents.FindByClass("prop_creepernest")
	for _, ent in ipairs(spawns) do
		ent.SpawnType = SPAWNTYPE_NEST
	end
	table.Add(GAMEMODE.CachedSpawns, spawns)

	spawns = ents.FindByClass("prop_darknest")
	for _, ent in ipairs(spawns) do
		ent.SpawnType = SPAWNTYPE_DARKNEST
	end
	table.Add(GAMEMODE.CachedSpawns, spawns)
end)
