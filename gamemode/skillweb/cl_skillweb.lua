...
net.Receive("zs_skills_init", function(length)
	...
	if MySelf:IsValid() then
		...
		MySelf:ApplySkills(active,true)
	end
end)
...
function GM:DrawXPBar(x, y, w, h, xpw, barwm, hm, level)
	...
	local append = ""
	if rlevel > 0 then
		append = translate.Format("skillweb_rlevel", rlevel)
	end
	...
	if level == GAMEMODE.MaxLevel then
		draw_SimpleText(translate.Get("skillweb_maxlevel")..append, "ZSXPBar", xpw / 2, h / 2 + y, COLOR_GREEN, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	else
		if progress > 0 then
...
function PANEL:Paint(w, h)
	...
	if sp > 0 then
		...
		draw_SimpleText(translate.Format("skillweb_sp_x", sp), "ZSHUDFontSmallest", w - 2, h / 2, colFlash, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end
...
/*
function GM:TogglePerkMenu()
	if self.PerkMenu and self.PerkMenu:IsValid() then
		self.PerkMenu:Remove()
		self.PerkMenu = nil
		return
	end

	self.PerkMenu = vgui.Create("ZSPerkMenu")
end
*/
...
function meta:SendDesiredActiveSkills(skills)
	//turn skills to an assoc
	local skills_assoc = table.ToAssoc(skills)
	print(table.ToString(skills_assoc))

	net.Start("zs_skills_desired")
	for skillid in pairs(GAMEMODE.Skills) do
		net.WriteBool(skills_assoc[skillid])
	end
	net.SendToServer()
end
