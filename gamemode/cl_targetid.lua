...
function GM:DrawTargetID(ent, fade)
	...
	local healthfraction = math_max(ent:Health() / (ent:Team() == TEAM_UNDEAD and ent:GetMaxHealth() or ent:GetMaxHealth()), 0)
	if healthfraction ~= 1 then
		...
		local hptxt = self.HealthTargetDisplay == 1 and translate.Format("targetid_hp", math_ceil(ent:Health())) or math_ceil(healthfraction * 100).."%"
		...
		if self.MedicalAura then
			...
			if poison >= 1 then
				...
				draw.SimpleTextBlur(translate.Format("targetid_poison", math.floor(poison)), "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
				y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
			end
			if bleed >= 1 then
				util.ColorCopy(COLOR_SOFTRED, colTemp)
				draw.SimpleTextBlur(translate.Format("targetid_bleed", math.floor(bleed)), "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
				y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
			end
			if phant >= 1 then
	...
	if ent:Team() == TEAM_UNDEAD then
		...
		if holding:IsValid() then
			draw.SimpleTextBlur(string_format(translate.Get("targetid_carrying"), string_match(holding:GetModel(), ".*/(.+)%.mdl") or "object"), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
		...
		if remortlevel >= 1 then
			draw.SimpleTextBlur(string_format(translate.Format("targetid_level_x_rlvl_x", level, remortlevel)), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
			draw.SimpleTextBlur(translate.Format("targetid_level_x", level), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		end
	end
end
...
function GM:DrawSigilTargetHint(ent, fade)
	...
	draw.SimpleTextBlur(translate.Get("targetid_sigil"), "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
	y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 0

	draw.SimpleTextBlur(translate.Get("targetid_sigil_press"), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
end
