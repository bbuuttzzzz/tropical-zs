...
function GM:GetResupplyTime()
	return self.ResupplyTime:GetFloat()
end

function GM:GetIsFreeplay()
	return self.FreeplayMode:GetBool()
end

function GM:GetManualRedeeming()
	return self.ManualRedeeming:GetBool()
end

function GM:GetRedeemFever()
	return self.RedeemFever:GetBool()
end
...
include("sh_statusfloaters.lua")
include("sh_RBPRegistry.lua")
...
GM.SoundDuration = {
	...
	["zombiesurvival/music_lose_tropical.ogg"] = 36.92,
	...
	["zombiesurvival/beats/tropical/1.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/2.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/3.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/4.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/5.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/6.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/7.ogg"] = 6.666,
	["zombiesurvival/beats/tropical/8.ogg"] = 6.666,

	["zombiesurvival_modded/newbeats/beta/1.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/2.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/3.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/4.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/5.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/6.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/7.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/8.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/9.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/10.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/11.ogg"] = 2,
	["zombiesurvival_modded/newbeats/beta/12.ogg"] = 2,

	["zombiesurvival_modded/newbeats/beta/b1.ogg"] = 8,
	["zombiesurvival_modded/newbeats/beta/b5.ogg"] = 8,
	["zombiesurvival_modded/newbeats/beta/b9.ogg"] = 8,
...
function GM:AddCustomAmmo()
	...
	game.AddAmmoType({name = "grenade_impulse"})
	game.AddAmmoType({name = "grenade_impact"})
	...
	game.AddAmmoType({name = "sigilseed"})
...
function GM:GetBestDynamicTeleport(pl, pos)
	local spawns = self:GetDynamicTeleports(pl)
	if #spawns == 0 then return end

	local bestSqrDist = false
	local best = false

	for i, spawn in pairs(spawns) do
		if not best then
			best = spawn
			bestSqrDist = pos:DistToSqr(spawn:GetPos())
			continue
		end

		local sqrDist = pos:DistToSqr(spawn:GetPos())
...
function GM:GetDynamicTeleports(pl)
	local tab = {}

	local allplayers = player.GetAll()
	for _, human in pairs(team.GetPlayers(TEAM_HUMAN)) do
		if human ~= pl and self:DynamicTeleportIsValid(human, allplayers) then
			table.insert(tab, human)
		end
	end

	return tab
end
function GM:DynamicTeleportIsValid(human, allplayers)
	-- I didn't make this check where trigger_hurt entities are. Rather I made it check the time since the last time you were hit with a trigger_hurt.
	-- I'm not sure if it's possible to check if a trigger_hurt is enabled or disabled through the Lua bindings.
	if SERVER and human.LastHitWithTriggerHurt and CurTime() < human.LastHitWithTriggerHurt + 2 then
		return false
	end
	...
	local pos = human:GetPos() + Vector(0, 0, 1)
	if human:Alive() and human:GetMoveType() == MOVETYPE_WALK and human:OnGround()
	and not util.TraceHull({start = pos, endpos = pos + playerheight, mins = playermins, maxs = playermaxs, mask = MASK_SOLID, filter = allplayers}).Hit then
		local vtr = util.TraceHull({start = pos, endpos = pos - playerheight, mins = playermins, maxs = playermaxs, mask = MASK_SOLID_BRUSHONLY})
		if not vtr.HitSky and not vtr.HitNoDraw then
			return true
...
function GM:DoChangeDeploySpeed(wep)
	if wep:IsValid() and wep.SetDeploySpeed and not wep.NoDeploySpeedChange then
		...
		wep:SetDeploySpeed(self.BaseDeploySpeed * (wep.DeploySpeedMultiplier or 1 )* (owner:IsValid() and owner.DeploySpeedMultiplier or 1) * (owner:IsValid() and owner:GetStatus("frost") and 0.7 or 1))
	end
end
	...
	if SERVER then
		...
		if groundent:IsValid() and groundent:IsPlayer() and PTeam(groundent) == TEAM_UNDEAD and pl:HasTrinket("curbstompers") then
			if groundent:IsHeadcrab() then
				...
				groundent:TakeSpecialDamage(groundent:Health() + 70, DMG_CLUB, pl, pl, pl:GetPos())
...
function GM:PlayerCanBeHealed(pl)
	...
	return pl:Health() < maxhp or pl:GetPoisonDamage() > 0 or pl:GetBleedDamage() > 0 or pl:GetRadiationDamage() > 0
end
...
function GM:PlayerCanPurchase(pl)
	...
	local canpurchase = PTeam(pl) == TEAM_HUMAN and pl:Alive() -- and self:GetWave() > 0 -- and pl:NearArsenalCrate()
...
local FEAR_RANGE = 1000^2
local FEAR_RANGE_BY_TYPE = {
	[ZTYPE_NORMAL] = FEAR_RANGE,
	[ZTYPE_MINIBOSS] = 750^2,
	[ZTYPE_BOSS] = 1500^2
}
local FEAR_PERINSTANCE = 0.1
local FEAR_BY_TYPE = {
	[ZTYPE_NORMAL] = FEAR_PERINSTANCE,
	[ZTYPE_MINIBOSS] = 0.5,
	[ZTYPE_BOSS] = 1
}
...
function GM:GetFearMeterPower(pos, teamid, ignore)
	...
	for _, pl in pairs(player.GetAll()) do
		if pl ~= ignore and PTeam(pl) == teamid and not pl:CallZombieFunction0("DoesntGiveFear") and pl:Alive() then
			...
			local range = FEAR_RANGE_BY_TYPE[classTab.Type]
			if dist <= range then
				power = power + (1 - dist / range) * (classTab.FearPerInstance or FEAR_BY_TYPE[classTab.Type] or FEAR_PERINSTANCE)
			end
		end
	end
...
--get this wave, but if we are in between waves get the next wave
function GM:GetNextActiveWave()
	//for the first frame of wave 1, it's wave 0 and the wave is active so... lol
	return (self:GetWave() == 0 and 1
		or self:GetWave() + (self:GetWaveActive() and 0 or 1))
end
...
function GM:GetNextResupply()
	return GetGlobalFloat("resupply", 0)
end

function GM:SetNextResupply(time)
	SetGlobalFloat("resupply", time)
end

function GM:GetNextReinforcement()
	return GetGlobalFloat("reinforcement")
end

function GM:SetNextReinforcement(time)
	SetGlobalFloat("reinforcement",time)
end

function GM:GetReinforcementCount()
	return GetGlobalInt("reinforcement_count")
end

function GM:SetReinforcementCount(count)
	SetGlobalInt("reinforcement_count", count)
...
function GM:GetWeaponResupplyType(wep)
  local ammotype = wep.GetResupplyAmmoType and wep:GetResupplyAmmoType() or wep.ResupplyAmmoType or wep:GetPrimaryAmmoTypeString()
  ammotype = ammotype and ammotype:lower() or nil

  return ((ammotype and self.AmmoResupply[ammotype]) and ammotype or nil)
end
