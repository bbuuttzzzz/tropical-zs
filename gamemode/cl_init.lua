...
include("cl_RBPParticleHandler.lua")
...
include("vgui/pperks.lua")
include("vgui/pupgrade.lua")
include("vgui/pmisc.lua")
...
include("vgui/zsweaponselect.lua")
...
function GM:HUDAmmoPickedUp(itemname, amount)
	timer.Simple(0.1, function()
		self:UpdateWeaponsByAmmo(string.lower(itemname))
	end)
end
...
function GM:DrawFearMeter(power, screenscale)
	...
	for i=1, #GAMEMODE.CachedSigils do
		sigil = sigils[i]
		health = 0
		maxhealth = 0
		...
		if health >= 0 then
			...
			if sigil and sigil:IsValid() then
				damageflash = 255 --math.min((CurTime() - sigil:GetSigilLastDamaged()) * 2, 1) * 255
			else
				damageflash = 255
			end
...
function GM:_Think()
	...
	if self:ShouldSendAFKMessage(time) then
		self:SendAFKMessage()
	end
	...
	--update player's equipped weapon ammo display
	self:UpdateWeaponEquipped()
end
...
function GM:HumanHUD(screenscale)
	...
	if self.IsUpgrade then
		draw_SimpleTextBlurry(translate.Get("press_f3_for_upgrades"), "ZSHUDFontSmall", w * 0.5, screenscale * 135, COLOR_GRAY, TEXT_ALIGN_CENTER)
	end
end
...
function GM:ZombieHUD()
	...
	if not self:GetWaveActive() and self:GetWave() ~= 0 then
		...
		local minis = GAMEMODE.NextMiniBosses
		local miniCount = minis and #minis or 0
		if miniCount > 1 then
			local x, y = ScrW() / 2, ScrH() * 0.3 + draw_GetFontHeight("ZSHUDFont") * 1.6
			--we have minibosses, so add a notice for that
			local isMini
			for _, n in ipairs(minis) do
				if n == MySelf then
					isMini = true
					break
				end
			...
			if isMini then
				--we're one of them
				draw_SimpleTextBlur(translate.Format("you_as_x_with_y_others",GAMEMODE.ZombieClasses[MySelf:GetSelectedMiniBossClass()].Name,miniCount-1),"ZSHUDFont", x, y, Color(255, 50, 50), TEXT_ALIGN_CENTER)
			else
				draw_SimpleTextBlur(translate.Format("x_minibosses_soon",miniCount),"ZSHUDFont", x, y, COLOR_GRAY, TEXT_ALIGN_CENTER)
			end
		elseif miniCount == 1 then
			local x, y = ScrW() / 2, ScrH() * 0.3 + draw_GetFontHeight("ZSHUDFont") * 1.6
			local isMini = (minis[1] == MySelf)

			if isMini then
				--we're one of them
				draw_SimpleTextBlur(translate.Format("you_as_x_soon",GAMEMODE.ZombieClasses[MySelf:GetSelectedMiniBossClass()].Name),"ZSHUDFont", x, y, Color(255, 50, 50), TEXT_ALIGN_CENTER)
			else
				draw_SimpleTextBlur(translate.Get("one_miniboss_soon"),"ZSHUDFont", x, y, COLOR_GRAY, TEXT_ALIGN_CENTER)
			end
		end
	end
end
...
function GM:DrawNestIndicators()
	...
	local k = 1
	for i, nest in pairs(GAMEMODE.CachedSpawns) do
		if not nest:IsValid() or nest.SpawnType ~= SPAWNTYPE_NEST then continue end
...
function GM:CreateLateVGUI()
	...
	if not self.WeaponSelectHUD then
		self.WeaponSelectHUD = vgui.Create("ZSWeaponSelect")
...
function GM:InitializeBeats()
	...
	for _, dirname in pairs(dirs) do
		...
		local lengthDir = "sound/zombiesurvival/beats/"..dirname.."/lengths.txt"
		local lengths
		if file.Exists(lengthDir, "GAME") then
			local contents = file.Read(lengthDir, "GAME")
			if contents and #contents > 0 then
				lengths = string.Explode(" ",contents)
			end
		end
		...
		for i=1, 10 do
			...
			if a1 then
				...
				if file.Exists("sound/"..filename, "GAME") then
					...
					if lengths then
						self.SoundDuration[filename] = lengths[i]
					end
...
function GM:LastHumanMessage()
	...
	if P_Team(MySelf) == TEAM_UNDEAD or not MySelf:Alive() then
		...
		if GAMEMODE:GetRedeemFever() then
			self:CenterNotify(COLOR_CYAN, translate.Get("Redeem Fever Activated"))
		end
	else
...
local function GiveWeapon()
	if GAMEMODE.HumanMenuLockOn then
		RunConsoleCommand("zsgiveweapon", GAMEMODE.HumanMenuLockOn:EntIndex())
	end
end
local function GiveWeaponClip()
	if GAMEMODE.HumanMenuLockOn then
		RunConsoleCommand("zsgiveweaponclip", GAMEMODE.HumanMenuLockOn:EntIndex())
	end
end
...
function GM:PlayerBindPress(pl, bind, wasin)
	...
	elseif wasin then
		if bind == "slot1" then
			self:SelectWeaponSlot(1)
			return true
		elseif bind == "slot2" then
			self:SelectWeaponSlot(2)
			return true
		elseif bind == "slot3" then
			self:SelectWeaponSlot(3)
			return true
		elseif bind == "slot4" then
			self:SelectWeaponSlot(4)
			return true
		elseif bind == "slot5" then
			self:SelectWeaponSlot(5)
			return true
		elseif bind == "slot6" then
			self:SelectWeaponSlot(6)
			return true
		elseif bind == "slot7" then
			self:SelectWeaponSlot(7)
			return true
		elseif bind == "slot8" then
			self:SelectWeaponSlot(8)
			return true
...
function GM:KeyPress(pl, key)
	if key == self.MenuKey then
		...
		elseif team == TEAM_ZOMBIE then --and not pl:Alive()
...
function GM:KeyRelease(pl, key)
	...
	elseif key == IN_SPEED then
		if pl:Alive() and P_Team(pl) == TEAM_UNDEAD then
...
function GM:ClearUpgrade()
	self.UpgradeItems = {}
	self.IsUpgrade = false
	self.UpgradeGroup = UPGRADEGROUP_NONE
	self:RegenerateUpgradeMenu()
end


function GM:DisplayUpgrade( _UpgradeGroup, _Items )
	self.UpgradeItems = _Items
	self.IsUpgrade = true
...
function GM:GuessNextUpgrade()
	local pts = MySelf:Frags()

	for key, tab in ipairs(self.UpgradeGroups) do
		if tab.points > pts then
			return key
		end
	end
...
function GM:TrySetClass(classtab, hard)
	if not self:ValidateZombieClass(classtab.Index, ZTYPE_NORMAL) then return end

	if not gamemode.Call("IsClassUnlocked", classtab.Index) then
		self:CenterNotify(COLOR_RED, translate.GetFormatted("class_not_unlocked_will_be_unlocked_x", classtab.Wave))
	else
		MySelf:SetSelectedZombieClass(classtab.Index, hard)
	end
end

function GM:TrySetBossClass(classtab)
	if not self:ValidateZombieClass(classtab.Index, ZTYPE_BOSS) then return end

	if not gamemode.Call("IsClassUnlocked", classtab.Index) then
		self:CenterNotify(COLOR_RED, translate.GetFormatted("class_not_unlocked_will_be_unlocked_x", classtab.Wave))
	else
		MySelf:SetSelectedBossClass(classtab.Index)
	end
end

function GM:TrySetMiniBossClass(classtab)
	if not self:ValidateZombieClass(classtab.Index, ZTYPE_MINIBOSS) then return end

	if not gamemode.Call("IsClassUnlocked", classtab.Index) then
		self:CenterNotify(COLOR_RED, translate.GetFormatted("class_not_unlocked_will_be_unlocked_x", classtab.Wave))
	else
		MySelf:SetSelectedMiniBossClass(classtab.Index)
	end
end

function GM:PlayerSwitchWeapon(player, oldWeapon, newWeapon)
	timer.Simple(0,function()
		self:UpdateWeapon(oldWeapon)
		self:UpdateWeapon(newWeapon)
	  self:UpdateAmmoFamily()
	end)
end

function GM:PlayerButtonDown(pl, button)
	if not AFK_DID_WARN and pl ~= MySelf then
		AFK_DID_WARN = true
		print("cl:PlayerButtonDown does not work as I thought. tell doobert.")
	end
	self.AFK_MessageSent = nil
	self.AFK_LastButtonPressed = CurTime()
end

function GM:ShouldSendAFKMessage(curtime)
	if (not self.AFK_MessageSent
		and self.AFK_LastButtonPressed
		and (self.AFK_LastButtonPressed + self.ZombieAFKTimer < curtime))
	then
		return true
	end
end

function GM:SendAFKMessage()
	self.AFK_MessageSent = true
	RunConsoleCommand("zs_afk")
end
