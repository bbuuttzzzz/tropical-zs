...
GM.NailHealthPerRepair = CreateConVar("zs_nailhealthperrepair", "50", FCVAR_ARCHIVE + FCVAR_NOTIFY, "How much health a nail gets when being repaired."):GetInt()
cvars.AddChangeCallback("zs_nailhealthperrepair", function(cvar, oldvalue, newvalue)
	GAMEMODE.NailHealthPerRepair = tonumber(newvalue) or 50
end)

GM.NailHealCooldownRegen = CreateConVar("zs_nailhealcooldownregen", "0.1", FCVAR_ARCHIVE + FCVAR_NOTIFY, "fraction of zs_nailhealcooldownmax to restore per second"):GetFloat()
cvars.AddChangeCallback("zs_nailhealcooldownmax", function(cvar, oldvalue, newvalue)
	GAMEMODE.NailHealCooldownRegen = tonumber(newvalue) or 0.1
end)
...
GM.MedkitPointsPerHealth = CreateConVar("zs_medkitpointsperhealth", "0.34", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Specifies the amount of points for players per health healed. For use with the medkit and such."):GetFloat()
...
GM.PointsPerRepairHealth = CreateConVar("zs_pointsperrepairhealth", "0.06", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Specifies the amount of points for players per health repaired. For use with nails and such."):GetFloat()
