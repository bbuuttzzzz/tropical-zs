...
function TrueVisibleFilters(posa, posb, ...)
	...
	TrueVisibleTrace.start = posa
	TrueVisibleTrace.endpos = posb
	TrueVisibleTrace.mask = MASK_SOLID_BRUSHONLY --MASK_SHOT
...
function util.BlastDamagePlayer(inf, att, center, radius, damage, damagetype, maxFalloffFrac, doIgnoreAttacker, force)
	if not att:IsValidPlayer() then ErrorNoHalt("[BlastDamagePlayer] Tried to use a nonplayer") end

	util.BlastDamageEx(inf, att, center, radius * (att.ExpDamageRadiusMul or 1), damage * (att.ExplosiveDamageMul or 1), damagetype, maxFalloffFrac, doIgnoreAttacker, force)
end

-- I had to make this since the default function checks visibility vs. the entitiy's center and not the nearest position.
function util.BlastDamageEx(inflictor, attacker, epicenter, radius, damage, damagetype, maxFalloffFrac, doIgnoreAttacker, force)
	maxFalloffFrac = maxFalloffFrac or 0.5
	--doIgnoreAttacker already defaults to nil -> false
	force = force or 0 --force of knockback

	for _, ent in pairs(ents.FindInSphere(epicenter, radius)) do
		if ent:IsValid() then

			if doIgnoreAttacker and ent == attacker then
				--this is the attacker and they shouldnt take blast damage
				continue
			end
			...
			if TrueVisibleFilters(epicenter, nearest, inflictor, attacker, ent)
				...
				--this is what fraction of damage to apply
				local frac = 1 - (nearest:Distance(epicenter)/radius * maxFalloffFrac)
				ent:TakeSpecialDamage(frac * damage, damagetype, attacker, inflictor, nearest)
				ent:ThrowFromPosition(epicenter, force * frac, true)
			end
		end
	end
end

function util.BlastDamageSingle(inflictor, attacker, target, epicenter, radius, damage, damagetype, maxFalloffFrac, force)
	if not attacker:IsValidPlayer() then ErrorNoHalt("[BlastDamagePlayer] Tried to use a nonplayer") end

	radius = radius * (attacker.ExpDamageRadiusMul or 1)
	damage = damage * (attacker.ExplosiveDamageMul or 1)
	maxFalloffFrac = maxFalloffFrac or 0.5
	force = force or 0

	if target:IsValid() then
		local nearest = target:NearestPoint(epicenter)

		--if this check fails, the target is out of range so they shouldn't take damage
		if nearest:Distance(epicenter) > radius then return end

		if TrueVisibleFilters(epicenter, nearest, inflictor, attacker, target)
			or TrueVisibleFilters(epicenter, target:EyePos(), inflictor, attacker, target)
			or TrueVisibleFilters(epicenter, target:WorldSpaceCenter(), inflictor, attacker, target) then

			--this is what fraction of damage to apply
			local frac = 1 - (nearest:Distance(epicenter)/radius * maxFalloffFrac)
			target:TakeSpecialDamage(frac * damage, damagetype, attacker, inflictor, nearest)
			target:ThrowFromPosition(epicenter, force * frac, true)
		end
	end
...
function util.FindVisibleInSphere(inflictor, attacker, epicenter, radius)
	local tab = {}
	local hiddenTab = {}
	for _, ent in pairs(ents.FindInSphere(epicenter, radius)) do
		if ent:IsValid() then
			...
			if TrueVisibleFilters(epicenter, nearest, inflictor, attacker, ent)
				...
				hiddenTab[#hiddenTab + 1] = ent
			end
		end
	end

	return tab, hiddenTab
end
...
--creates t from supplied x values,
--then feeds it into Lerp(t,from,to)
function Lerp2(x, xmin, xmax, from, to)
	local t = (x - xmin)/(xmax - xmin)
	return Lerp(t, from, to)
end

--scales melee damage based on number of players hit
function CalcMeleeDamageScale(numplayers)
	return math.Clamp(1.1 - numplayers * 0.1, 0.666, 1)
end

--takes a table of traces and finds out how many of them hit a player
function GetTracesNumPlayers(traces)
	local numplayers = 0
...
--gets the amount of projectiles from a numshots
function GetTrueBulletCount(count)
	if count and count >= 0 then return count end

	if BULLETPATTERNS[count] then
