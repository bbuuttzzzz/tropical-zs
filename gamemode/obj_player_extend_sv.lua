...
function meta:ProcessDamage(dmginfo)
	...
	if P_Team(self) == TEAM_UNDEAD then
		--zombies are immune if they are in zombie spawn
		--...unless it's waveburn
		if self.SpawnProtection and not dmgbypass then
			dmginfo:SetDamage(0)
		...
		if attacker:IsValidLivingHuman() and inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
			...
			if wep.IsMelee then
				...
				if attacker:IsSkillActive(SKILL_HEAVYSTRIKES) and not self:GetZombieClassTable():IsBoss() and (wep.IsFistWeapon and attacker:IsSkillActive(SKILL_CRITICALKNUCKLE) or wep.MeleeKnockBack > 0) then
...
function meta:GetBossZombieIndex()
	return self:GetSelectedBossClass()
	/*
	local bossclasses = {}
	for _, classtable in pairs(GAMEMODE.ZombieClasses) do
		if classtable:IsBoss() then
...
function meta:TrySpawnAsGoreChild(ent)
	if ent and ent:IsValid() and not ent.SpawnedOn and ent:GetSettled() then
		...
		self:BecomeClass(ent:GetClass() == "prop_thrownbaby" and "Gore Child" or "Shadow Child")
...
function meta:AddPoisonDamage(damage, attacker)
	...
	if status and status:IsValid() then
		damage = status:AddDamage(damage, attacker)
		GAMEMODE:StatusFloater(attacker,self,vector_origin,damage,"poison")
	end
end
...
function meta:AddBleedDamage(damage, attacker)
	...
	if status and status:IsValid() then
		damage = status:AddDamage(damage, attacker)
		GAMEMODE:StatusFloater(attacker,self,vector_origin,damage,"bleed")
	end
end

function meta:AddRadiationDamage(damage, attacker)
	local status = self:GiveStatus("radiation")
	if status and status:IsValid() then
		damage = status:AddDamage(damage, attacker)
		GAMEMODE:StatusFloater(attacker,self,vector_origin,damage,"radiation")
	end
end

function meta:AddCrippleDamage(damage, attacker)
	local status = self:GiveStatus("cripple", -2)
	if status and status:IsValid() then
		damage = status:AddDamage(damage)
		GAMEMODE:StatusFloater(attacker,self,vector_origin,damage,"cripple")
	end
end
...
function meta:ChangeToCrow()
	...
	self:BecomeClass(crowclass.Index)
end

function meta:BecomeBoss(classindex)
	self:BecomeClass(classindex)
...
function meta:BecomeClass(classindex, loud)
	if loud and GAMEMODE.ZombieClasses[classindex].Type == ZTYPE_BOSS then
		self:BecomeBoss(classindex)
		return
	end

	self:SetZombieClass(classindex)
	self:DoHulls(classindex, TEAM_UNDEAD)
	self.DeathClass = classindex
	self:UnSpectateAndSpawn()

	if GAMEMODE.ZombieClasses[classindex].Type == ZTYPE_MINIBOSS then
		net.Start("zs_miniboss_spawned")
			net.WriteUInt(classindex, 8)
		net.Send(self)
	end
end
function meta:RespawnDelayed(nest)
	if not self:IsValidLivingZombie() or not GAMEMODE:GetWaveActive() then return end

	self:Kill()

	timer.Create("RespawnDelayed"..self:EntIndex(),1,2,function()
		self.ForceDynamicSpawn = nest --if a nest isn't supplied, this will be nil and a normal spawn is used
		self:UnSpectateAndSpawn()
		if nest then
			self:RemoveStatus("zombiespawnbuff")
		end
	end)
end
...
function meta:Give(weptype, noammo)
	...
	local numWeps = table.Count(weps)

	local didSwap = false
	if numWeps >= GAMEMODE.WeaponSlots then
		--we already have 8 weapons, so drop our currently held weapon first
		local ent = self:DropWeaponByType(self:GetActiveWeapon():GetClass())
		if ent and ent:IsValid() then
			local shootpos = self:GetShootPos()
			local aimvec = self:GetAimVector()
			ent:SetPos(util.TraceHull({start = shootpos, endpos = shootpos + aimvec * 32, mask = MASK_SOLID, filter = self, mins = Vector(-2, -2, -2), maxs = Vector(2, 2, 2)}).HitPos)
...
local OldStrip = meta.StripWeapon
function meta:StripWeapon(weapon)
	self:RefreshWeaponSelect()
	return OldStrip(self, weapon)
end
...
/*
creates a status from /entities/entities/status_<sType>, overriding existing
status of same type if it exists, lasting a duration of fDie
fDie = -1: last 9999999999 seconds
fDie = 0: last -(server uptime) seconds (dies on next frame)
else: lasts fDie seconds

returns: the entity status
*/
function meta:GiveStatus(sType, fDie, giver)
...
function meta:SecondWind(pl)
	...
	self:BecomeClass(self.DeathClass or self:GetZombieClass())
...
function meta:DropAmmoByType(ammotype, amount)
	...
	if ent:IsValid() then
		...
		self:UpdateAmmoAmount(ammotype)
		return ent
	end
end
...
function meta:AddPoints(points, floatingscoreobject, fmtype, nomul)
	...
	--handle fractional scrap before we add it
	local scrap = wholepoints * (self.ScrapGainMul or 1)
	local wholescrap = math.floor(scrap)
	local remainder = scrap - wholescrap
	if remainder > 0 then
		self.ScrapRemainder = self.ScrapRemainder + remainder
		local carryover = math.floor(self.ScrapRemainder)
		wholescrap = wholescrap + carryover
		self.ScrapRemainder = self.ScrapRemainder - carryover
	end

	self:AddFrags(wholepoints) --this is adding score
	self:AddScrap(wholescrap) --this is adding scrap
...
function meta:AddScrap(scrap)
	self:GiveAmmo(scrap,"scrap",true)
end
function meta:TakeScrap(scrap)
	self:RemoveAmmo(scrap,"scrap")
end
function meta:SetScrap(scrap)
	self:SetAmmo(scrap,"scrap")
end
...
function meta:SetZombieClass(index, onlyupdate, filter)
	if onlyupdate then
		net.Start("zs_zclass")
			net.WriteEntity(self)
	...
	local classtab = GAMEMODE.ZombieClasses[index]
	if classtab then
		self.Class = index
		if P_Team(self) == TEAM_UNDEAD then
			self:DoHulls(index)
...
function meta:Redeem(silent, noequip)
	...
	self:KillSilent()
	self:RemoveEphemeralStatuses()
	self:Extinguish()
	self:SetPhantomHealth(0)
...
function meta:RedeemNextFrame(isManualRedeem,isForceRedeem)
	timer.Simple(0, function()
		if IsValid(self) then
			self:CheckRedeem(true,isManualRedeem,isForceRedeem)
		end
	end)
end
...
function meta:CheckRedeem(instant, isManualRedeem, isForceRedeem)

	local manual = GAMEMODE:GetManualRedeeming()
	local redeemBrains = GAMEMODE:GetRedeemFever() and LASTHUMAN and 2 or GAMEMODE:GetRedeemBrains()

	--a valid redeem automatically triggers if it's last human or last wave
	--also an incoming "true" should happen if this is the redeemnextframe redeem,
	--or if it's the wave end redeem
	isForceRedeem = (isForceRedeem or LASTHUMAN
		or (manual and (GAMEMODE:GetWave() == GAMEMODE:GetNumberOfWaves() or not GAMEMODE:GetWaveActive())))

	if not self:IsValid() or P_Team(self) ~= TEAM_UNDEAD
	or GAMEMODE:GetRedeemBrains() <= 0 or self:GetBrains() < redeemBrains
	or GAMEMODE.NoRedeeming or self.NoRedeeming then return end

	if not isForceRedeem and self:GetZombieClassTable():IsBoss() then
		if manual and isManualRedeem then
			self:CenterNotify(COLOR_RED, translate.ClientGet(self, "boss_cannot_redeem"))
		end
		return
	end

	if not manual or isManualRedeem or isForceRedeem then
		if instant then
			self:Redeem()
		else
			self:RedeemNextFrame(isManualRedeem, isForceRedeem)
		end
	else
		net.Start("zs_can_redeem")
		net.Send(self)
	end
	return true
end
	...
	if ent then
		if ent ~= self then
			if ent:IsPlayer() and not ent:IsBot() then
				self.LastPlayerAttacker = ent
				self.LastPlayerAttacked = CurTime()
			end
...
function meta:DoSigilTeleport(target, from)
	if not target then return end

	if from and not from:IsValid() --if the thing that teleported you is null
		or not from:IsWeapon() and self:GetPos():DistToSqr(from:GetPos()) > 16384 --if you use a sigil shard and you are really close
		or from:IsWeapon() and self:GetActiveWeapon() ~= from then --if you use a sigil shard but switched weapons
		return
	end

	if self:IsValidLivingHuman() and target:IsValid() then
		if CurTime() >= (self._NextSigilTeleportEffect or 0) then
			self._NextSigilTeleportEffect = CurTime() + 0.25

			local effect = "sigil_teleport"
...
function meta:DoBackupPlanTeleport(from)
	...
	local target = GAMEMODE:GetBestDynamicTeleport(self, self:GetPos())

	if self:IsValidLivingHuman() and target and target:IsValid() then
		if CurTime() >= (self._NextSigilTeleportEffect or 0) then
			self._NextSigilTeleportEffect = CurTime() + 0.25
...
function meta:PulseResonance(attacker, inflictor)
	...
	timer.Create("PulseResonance" .. attacker:UniqueID(), 0.06, 1, function()
		...
		if attacker:IsValidLivingHuman() then
			util.BlastDamagePlayer(inflictor, attacker, pos, 100, 75, DMG_ALWAYSGIB)
...
function meta:CryogenicInduction(attacker, inflictor, damage)
	if self:Health() > self:GetMaxHealth() * (damage/100) or math.random(50) > damage then return end
	...
	timer.Create("Cryogenic" .. attacker:UniqueID(), 0.06, 1, function()
		...
		--was DMG_DIRECT. changed so it doesn't break spawn protection. shouldn't break anything
		self:TakeSpecialDamage(self:Health() + 90, DMG_GENERIC, attacker, inflictor, pos)

		if attacker:IsValidLivingHuman() then
			util.BlastDamagePlayer(inflictor, attacker, pos, 100, self:GetMaxHealth() * 0.12, DMG_DROWN)
			for _, ent in pairs(util.BlastAlloc(inflictor, attacker, pos, 100 * (attacker.ExpDamageRadiusMul or 1))) do
				if ent:IsValidLivingPlayer() and gamemode.Call("PlayerShouldTakeDamage", ent, attacker) then
...
function meta:SetSelectedZombieClass(index)
	self:SetDTInt(DT_PLAYER_INT_CLASS,index)
end
function meta:SetSelectedBossClass(index)
	self:SetDTInt(DT_PLAYER_INT_BOSS,index)
end

function meta:SetSelectedMiniBossClass(index)
	self:SetDTInt(DT_PLAYER_INT_MINI_BOSS,index)
end

function meta:RefreshWeaponSelect()
	net.Start("zs_reset_weapon_select")
	net.Send(self)
end

function meta:UpdateAmmoAmount(ammoType)

	--convert to ID if it isn't already an ID.
	local id = game.GetAmmoID(ammoType)
	net.Start("zs_dropped_ammo")
		net.WriteUInt(id ~= -1 and id or ammoType,16)
	net.Send(self)
end

function meta:ActivateHumanSkills()
	self.ActivatedHumanSkills = true
	self:ApplySkills()
	self:GiveSkillItems()
end
...
--sets oomph to be this fraction of full
function meta:SetOomph(amount)
	local fraction = amount/self:GetMaxOomph()
	self.OomphTimeMax = CurTime() + (1-fraction)*self:GetOomphChargeTime()


	net.Start("zs_update_oomph")
		net.WriteFloat(self:GetOomph())
	net.Send(self)
end

function meta:StartSoftControl()
	if not D3bot or not D3bot.IsEnabled or not self.D3bot_InitializeOrReset then return end

	self:D3bot_InitializeOrReset(true)

	net.Start("zs_afk_popup")
	net.Send(self)
end
