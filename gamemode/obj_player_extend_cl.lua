...
function meta:SetZombieClass(index)
	self:CallZombieFunction0("SwitchedAway")

	local classtab = GAMEMODE.ZombieClasses[index]
	if classtab then
		self.Class = classtab.Index or index
		self:CallZombieFunction0("SwitchedTo")
	end
end

function meta:GetRateOfPalsy(ft, frightened, health, threshold, gunsway)
	local healthth = health < threshold and (math.min(1,(threshold - health) / (GAMEMODE.DefaultCalmThreshold-GAMEMODE.DefaultMinCalm)) * 7) or 0
...
function meta:SetSelectedZombieClass(index, hard)
	self:SetDTInt(DT_PLAYER_INT_CLASS,index)
	net.Start("zs_changeclass")
		net.WriteUInt(index,8)
		net.WriteUInt(ZTYPE_NORMAL,8)
		net.WriteBool(hard)
	net.SendToServer()
end
...
function meta:SetSelectedMiniBossClass(index)
	...
	net.Start("zs_changeclass")
		net.WriteUInt(index,8)
		net.WriteUInt(ZTYPE_MINIBOSS,8)
	net.SendToServer()
end
