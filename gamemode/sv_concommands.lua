...
concommand.Add("zs_shopbuy", function(sender, command, arguments)
	if not (sender:IsValid() and sender:IsConnected()
		and sender:IsValidLivingHuman()) or #arguments == 0 then return end
	...
	local signature = arguments[1]
	local shopTab = GAMEMODE.ShopItems[signature]

	if not shopTab then return end

	local scrap = sender:GetScrap()
	local cost = shopTab.price

	if scrap < cost then
		GAMEMODE:ConCommandErrorMessage(sender, translate.ClientGet(sender,"dont_have_enough_scrap"))
		return
	end

	sender:TakeScrap(cost)

	sender:SendLua("surface.PlaySound(\"ambient/levels/labs/coinslot1.wav\")")

	local itemTab = GAMEMODE.Items[signature]

	sender:GiveItem( itemTab )
...
concommand.Add("zs_pointsshopbuy", function(sender, command, arguments)
	...
	--this checks if the player is close enough to the arsenal crate, since there aren't arsenal crates who cares
	--[[
...
concommand.Add("zs_claimstockpile", function(sender, command, arguments)
	if not (sender:IsValid() and sender:Alive() and sender:Team() == TEAM_HUMAN)
		or not sender.MaxStockpiles or not sender.Stockpiles or sender.Stockpiles < 1
		then return end
	...
	net.Start("zs_stockpile")
		net.WriteUInt(sender.Stockpiles, 8)
	net.Send(sender)
...
concommand.Add("zs_selectupgrade", function(sender, command, arguments)
	if(CurrentUpgradeGroup == UPGRADEGROUP_NONE) then
		return
	end

	local card = tonumber(arguments[1])
	GAMEMODE:SelectUpgrade(sender, card)
	GAMEMODE:CheckUpgrades(sender, true)
end)

concommand.Add("zs_afk", function(sender, command, arguments)
	if not sender:IsValidZombie() then return end

	sender:StartSoftControl()
end)
