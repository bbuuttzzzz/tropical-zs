...
function PANEL:OpenMenu()
	...
	self.OnKeyCodePressed = function(self, keycode)
		...
		if bind == "slot1" then
			GAMEMODE:SwapEquippedWithSlot(1)
			return true
		elseif bind == "slot2" then
			GAMEMODE:SwapEquippedWithSlot(2)
			return true
		elseif bind == "slot3" then
			GAMEMODE:SwapEquippedWithSlot(3)
			return true
		elseif bind == "slot4" then
			GAMEMODE:SwapEquippedWithSlot(4)
			return true
		elseif bind == "slot5" then
			GAMEMODE:SwapEquippedWithSlot(5)
			return true
		elseif bind == "slot6" then
			GAMEMODE:SwapEquippedWithSlot(6)
			return true
		elseif bind == "slot7" then
			GAMEMODE:SwapEquippedWithSlot(7)
			return true
		elseif bind == "slot8" then
			GAMEMODE:SwapEquippedWithSlot(8)
			return true
		elseif bind == "invnext" then
			GAMEMODE:SwapNextWeapon()
			return true
		elseif bind == "invprev" then
			GAMEMODE:SwapPrevWeapon()
			return true
		end
	end
