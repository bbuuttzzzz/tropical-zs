...
local function SwitchPlayerModel(self)
	...
	chat.AddText(COLOR_LIMEGREEN, translate.Format("pm_selection_message", tostring(self.m_ModelName)))
...
function MakepPlayerModel()
	...
	pPlayerModel:SetTitle(translate.Get("pm_selection_label"))
...
function MakepPlayerColor()
	...
	local label = EasyLabel(pPlayerColor, translate.Get("pc_colors_label"), "ZSHUDFont", color_white)
	label:SetPos((pPlayerColor:GetWide() - label:GetWide()) / 2, y)
	y = y + label:GetTall() + 8

	local lab = EasyLabel(pPlayerColor, translate.Get("pc_player_color"))
	lab:SetPos(8, y)
	y = y + lab:GetTall()
	...
	lab = EasyLabel(pPlayerColor, translate.Get("pc_weapon_color"))
	lab:SetPos(8, y)
	y = y + lab:GetTall()
...
function GM:ShowHelp()
	...
	menu.OnKeyCodePressed = function(self, keycode)
	...
	but:SetText(translate.Get("pmainmenu_help"))
	...
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_plmodel"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_plcolor"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_options"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_swepdata"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_perks"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	but.DoClick = function()
		menu:Remove()
		GAMEMODE:OpenPerkMenu()
	end

	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_credits"))
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText(translate.Get("pmainmenu_close"))
	but:SetTall(buttonhei)
