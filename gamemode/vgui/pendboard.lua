...
function MakepEndBoard(winner)
	...
	if localwin then
		...
		heading = EasyLabel(frame, translate.Get("pendboard_won"), "ZSHUDFontBig", COLOR_CYAN)
	else
		surface.PlaySound("ambient/levels/citadel/strange_talk"..math.random(3, 11)..".wav")
		heading = EasyLabel(frame, translate.Get("pendboard_lost"), "ZSHUDFontBig", COLOR_RED)
	end
	...
	local subheading
	if localwin then
		subheading = EasyLabel(frame, translate.Get("pendboard_humans"), "ZSHUDFontSmaller", COLOR_WHITE)
	else
		subheading = EasyLabel(frame, translate.Get("pendboard_zombies"), "ZSHUDFontSmaller", COLOR_LIMEGREEN)
	end
	subheading:SetPos(wid * 0.5 - subheading:GetWide() * 0.5, y)
	y = y + subheading:GetTall() + 2

	local svpan = EasyLabel(frame, translate.Get("pendboard_mentions"), "ZSHUDFontSmall", COLOR_WHITE)
...
function PANEL:SetPlayer(pl, col, misc, misccol, overridename)
	...
	if pl:IsValidPlayer() then
		...
		avatar:SetPlayer(pl)
		avatar:SetTooltip(translate.Get("pendboard_steam"))
