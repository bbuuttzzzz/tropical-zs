...
function meta:GetRadiationDamage()
	return self.Radiation and self.Radiation:IsValid() and self.Radiation:GetDamage() or 0
end

function meta:HealPoisonDamage(damage)
	local status = self:GetStatus("poison")
	if status and status:IsValid() then
		return status:ApplyHeal(damage)
	else
		return damage
	end
end

function meta:HealBleedDamage(damage)
	local status = self:GetStatus("bleed")
	if status and status:IsValid() then
		return status:ApplyHeal(damage)
	else
		return damage
...
function meta:HealRadiationDamage(damage)
	local status = self:GetStatus("radiation")
	if status and status:IsValid() then
		return status:ApplyHeal(damage)
	else
		return damage
...
function meta:NearArsenalCrate()
	...
	return false--]]
	error("attempted to check for arsenal crate",1)
	return true
end
...
function meta:NearRemantler()
	...
	return false
	--]]
	error("attempted to check for remantler",1)
	return true
end
...
function meta:SetZombieClassName(classname)
	if GAMEMODE.ZombieClasses[classname] then
		self:BecomeClass(GAMEMODE.ZombieClasses[classname].Index)
...
function meta:IsBoss()
	return self:GetZombieClassTable():IsBoss()
end
...
function meta:ResetSpeed(noset, health)
	...
	if not speed then
		speed = GAMEMODE.PlayerSpeed - (self.WeaponWeightMul or 1) * (wep.WeightClass or 0)
	end
	...
	--DAMAGED SPEED DECREASE
	if 32 < speed then
		health = health or self:Health()
		local threshold = (self.CalmThreshold or GAMEMODE.DefaultCalmThreshold)
		if health < threshold then
			speed = math.max(88, speed - GAMEMODE.MaxCalmSlow * speed * math.min(1,(threshold - health) / (GAMEMODE.DefaultCalmThreshold - GAMEMODE.DefaultMinCalm)))
		end
	end
...
function meta:SetBarricadeGhosting(b, fullspeed)
	...
	if b and not self:GetBarricadeGhosting() then
		self:SetDTFloat(DT_PLAYER_PHASE_DECAY, CurTime() + GAMEMODE.PhaseDecayDuration)
	end
...
//previously we were using the max HP in the zombie class table to find the
//zombie's max HP. i think this was probably for optimization. now that each
//call has to do slightly more math, I bet it's faster to just use actual zombie
//max HP
function meta:GetMaxZombieHealth()
	return self:GetZombieClassTable().CalcMaxHealth()
end
/*
...
--this returns the PLAYER SET respawn zombie class, not their current class

function meta:IsClassSelected(index)
	return (
		index == self:GetSelectedZombieClass()
		or index == self:GetSelectedBossClass()
		or index == self:GetSelectedMiniBossClass()
		)
end

function meta:GetSelectedZombieClass()
	local c = self:GetDTInt(DT_PLAYER_INT_CLASS)
	return (c ~= 0 and c or GAMEMODE.DefaultZombieClass)
end
function meta:GetSelectedBossClass()
	local c = self:GetDTInt(DT_PLAYER_INT_BOSS)
	return (c ~= 0 and c or GAMEMODE.DefaultBossClass)
end
function meta:GetSelectedMiniBossClass()
	local c = self:GetDTInt(DT_PLAYER_INT_MINI_BOSS)
	return (c ~= 0 and c or GAMEMODE.DefaultMiniBossClass)
end

function meta:GetClassType()
	return self:GetZombieClassTable().Type
end

function meta:IsBossOrMini()
	local t = self:GetClassType()

	return t == ZTYPE_BOSS or t == ZTYPE_MINIBOSS
end
...
function meta:GetScrap()
	return self:GetAmmoCount("scrap")
end

function meta:GetOomph()
	if not self.OomphTimeMax then self.OomphTimeMax = CurTime() end

	if CurTime() >= self.OomphTimeMax then return self:GetMaxOomph() end

	local fraction = 1 - (self.OomphTimeMax-CurTime())/self:GetOomphChargeTime()

	return fraction * self:GetMaxOomph()
end

function meta:GetMaxOomph()
	return (self.OomphMaxMul or 1) * GAMEMODE.OomphMaxDefault
end

function meta:GetOomphChargeTime()
	return (self.OomphChargeTimeMul or 1) * GAMEMODE.OomphChargeTimeDefault
end
