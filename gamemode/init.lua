...
AddCSLuaFile("sh_statusfloaters.lua")
AddCSLuaFile("sh_RBPRegistry.lua")
...
AddCSLuaFile("cl_RBPParticleHandler.lua")
...
AddCSLuaFile("vgui/pmisc.lua")
AddCSLuaFile("vgui/pshop.lua")
AddCSLuaFile("vgui/pperks.lua")
AddCSLuaFile("vgui/pupgrade.lua")
AddCSLuaFile("vgui/premantle.lua")
AddCSLuaFile("vgui/zshealtharea.lua")
AddCSLuaFile("vgui/zsweaponselect.lua")
...
function GM:HumanCanCarry(pl, entity)
	if self.ZombieEscape or pl.NoObjectPickup or not pl:Alive() or pl:Team() ~= TEAM_HUMAN or (entity.NoPickupsTime and CurTime() < entity.NoPickupsTime and entity.NoPickupsOwner ~= pl) then return end
...
function GM:AddResources()
	...
	resource.AddFile("materials/tropical/stat_dps.png")
	resource.AddFile("materials/tropical/stat_dpr.png")
	resource.AddFile("materials/tropical/stat_range.png")
	resource.AddFile("materials/tropical/stat_utility.png")
...
function GM:Initialize()
	...
	self:AddCustomAmmo()
	self:CreateWeaponQualities() --stupid bullshit
	self:AddNetworkStrings()
	self:RegisterFood()
	self:LoadProfiler() --stupid bullshit?
...
function GM:AddNetworkStrings()
	...
	util.AddNetworkString("zs_reinforcements")
	...
	util.AddNetworkString("zs_dmg_status")
	...
	util.AddNetworkString("zs_stockpile")
	...
	util.AddNetworkString("zs_buyclass")
	...
	util.AddNetworkString("zs_map_table_init")
	...
	util.AddNetworkString("zs_upgrade_unlocked")
	util.AddNetworkString("zs_upgrade_choose")
	util.AddNetworkString("zs_upgrade_clear") --clear clientside upgrade stuff
	util.AddNetworkString("zs_glory_heal")
	util.AddNetworkString("zs_waveend_heal")
	util.AddNetworkString("zs_braincells_set")
	...
	util.AddNetworkString( "RBProjectileDestroyed" )
end
...
function GM:ShowTeam(pl)
	if pl:Team() == TEAM_HUMAN and not self.ZombieEscape then
		if true then --self:GetWave() > 0
			pl:SendLua("GAMEMODE:OpenShopMenu()")
			--pl:SendLua("GAMEMODE:OpenArsenalMenu()")
		end
	end

	if pl:Team() == TEAM_ZOMBIE then
		pl:CheckRedeem(false,true,false)
	end
end
...
function GM:ShowSpare1(pl)
	...
	elseif pl:Team() == TEAM_HUMAN then
		pl:SendLua("GAMEMODE:OpenUpgradeMenu()")
	end
end
...
function GM:PlayerPointsAdded(pl, amount)
	self:CheckUpgradeOnPointAdd( pl )
end
...
weaponmodelstoweapon["models/props_canal/mattpipe.mdl"] = "weapon_zs_golfclub"
...
function GM:PlayerSelectSpawn(pl)
	...
	elseif teamid == TEAM_UNDEAD then
		if pl:GetZombieClassTable():IsBoss() and (not pl.DeathClass or self.ZombieClasses[pl.DeathClass]:IsBoss()) and #self.BossSpawnPoints >= 1 then
...
local function BossZombieSort(za, zb)
	local ascore = za.WaveBarricadeDamage * 0.05 + za.WaveHumanDamage - (za:IsBot() and 500 or 0)
	local bscore = zb.WaveBarricadeDamage * 0.05 + zb.WaveHumanDamage - (zb:IsBot() and 500 or 0)
...
function GM:SpawnBossZombie(bossplayer, silent, bossindex, triggerboss)
	self:SpawnBoss(bossplayer,false,false,silent,bossindex,triggerboss)
end
...
function GM:Think()
	...
	if not self.RoundEnded then
		if self:GetWaveActive() then
			if self:GetWaveEnd() <= time and self:GetWaveEnd() ~= -1 then
				--if the round isn't ended, the wave is active, and it's past the wave end time,
				--end the wave
				gamemode.Call("SetWaveActive", false)
			end
		elseif self:GetWaveStart() ~= -1 then
			if self:GetWaveStart() <= time then
				--if the round IS ended and it's past the wave start time,
				--start the wave
			...
			and self.LastBossZombieSpawned ~= wave and wave > 0 and not self.RoundEnded then
				if self:GetWaveStart() - 5 <= time then
					--spawn bosses and minibosses
						if self:ShouldSpawnBoss() then
						self:SpawnBoss(nil,false)
					end
					self:SpawnMiniBosses(self:GetMiniBossCount(),true,false,true)
					self.LastBossZombieSpawned = wave
				else
					self:CalculateNextBosses()
				end
			end
		end
	end

	if self:GetWave() > 0 and self:GetNextResupply() <= time then
		self:DoResupply()
	end

	local r = self:GetNextReinforcement()
	if self:GetWave() > 0 and r ~= 0 and r <= time then
		self:DoReinforcements()
	end
	...
	self:HandleRayBasedProjectiles()
end
...
--whoa this gets called every frame that the wave is ended??? maybe we shouldn't do it that often...
--only need to do it right before we spawn actual bosses, and once every few seconds to keep the
--clients updated
GM.LastCalculatedBossTime = 0
GM.LastNumMVPs = 0
function GM:CalculateNextBosses()
	local mvps = self:GetMVPZombies(true,true)

	local ShouldSpawnBoss = self:ShouldSpawnBoss()
	local miniCount = self:GetMiniBossCount()
	local mvpCount = miniCount + (ShouldSpawnBoss and 1 or 0)

	--curb off MVPs if there aren't actually enough valid players to fill all the slots
	if mvpCount > #mvps then
		local diff = mvpCount - #mvps
		if diff <= miniCount then
			--we have an MVP to be boss still, so just remove minibosses
			miniCount = miniCount - diff
		else
			--we don't have ANY MVPs, so remove all of them
	...
	--if we are going to spawn a boss, make sure that player wants to be a boss.
	if ShouldSpawnBoss then
		self:FixMVPList(mvps)
	end

	--check if the list of MVPs has changed
	local hasChanged
	if self.LastNumMVPs ~= mvpCount or not not self.LastCalculatedMVPs then
		hasChanged = true
	else
		for n = 1, mvpCount do
			if mvps[n] ~= self.LastCalculatedMVPs[n] then
				hasChanged = true
				break
			end
		end
	...
	if hasChanged or CurTime() >= self.LastCalculatedBossTime + 2 then
		self.LastCalculatedMVPs = mvps
		self.LastNumMVPs = mvpCount
		self.LastCalculatedBossTime = CurTime()

		net.Start("zs_nextboss")
		if ShouldSpawnBoss and mvps[1] and mvps[1]:IsValid() then
			net.WriteEntity(mvps[1])
			net.WriteUInt(mvps[1]:GetSelectedBossClass(), 8)
		else
			net.WriteEntity(NULL)
			net.WriteUInt(1, 8)
		end
		net.WriteUInt(mvpCount,8)
		for n = 1, miniCount do
			net.WriteEntity(mvps[n + (ShouldSpawnBoss and 1 or 0)])
		end
		net.Broadcast()
	end
...
function GM:GetMiniBossCount()
	return math.ceil(player.GetCount() / self.PlayersPerMiniboss)
end
function GM:ShouldSpawnBoss()
	return (self.BossZombiePlayersRequired <= 0 or player.GetCount() >= self.BossZombiePlayersRequired)
end

--force noboss players off the first slot
function GM:FixMVPList(mvps)
	if mvps[1]:GetInfo("zs_nobosspick") == "0" or mvps[1]:IsBot() then return end
	--the player in slot 1 doesnt wanna be a boss, so move someone else up there

	for i, ent in ipairs(mvps) do
		if i == 1 then continue end

		if ent:GetInfo("zs_nobosspick") == "0" or ent:IsBot() then
			table.remove(mvps,i)
			table.insert(mvps,1,ent)
			return
		end
	end
end

function GM:GetMVPZombies(includeNoBossPlayers, includeBosses)
	local zombies = {}
	for _, ent in pairs(team.GetPlayers(TEAM_UNDEAD)) do
		if ((includeNoBossPlayers or ent:GetInfo("zs_nobosspick") == "0" or ent:IsBot())
		and (includeBosses or ent:GetClassType() ~= ZTYPE_BOSS)) then
...
function GM:GetMVPZombie(includeNoBossPlayers, includeBosses)
	local zombies = self:GetMVPZombies(includeNoBossPlayers, includeBosses)
	return zombies[1]
end
...
function GM:CalculateInfliction(victim, attacker)
	...
	--i removed the min check because i have to do something else whenever it
	--changes anyway so might as well only check once
	local infliction = zombies / players
	if infliction > self.CappedInfliction then
		if self.CappedInfliction < self.DoubleResupplyInfliction and infliction >= self.DoubleResupplyInfliction then
			for _, pl in pairs(player.GetAll()) do
				pl:CenterNotify(COLOR_RED, translate.ClientFormat(pl, "infliction_reached", self.DoubleResupplyInfliction * 100))
				pl:CenterNotify(COLOR_RED, translate.ClientFormat(pl, "ammo_generation_doubled"))
			end
		end
...
function GM:InitPostEntityMap(fromze)
	...
	--create sigils on initial map load
	--dont actual do it, and set use sigils to false as well
...
function GM:PlayerInitialSpawnRound(pl)
	...
	if pl:Team() == TEAM_UNDEAD and not self:GetWaveActive() then
		...
		pl:BecomeClass(self.DefaultZombieClass)
	end
...
function GM:EntityTakeDamage(ent, dmginfo)
	...
	--this is the zombie getting back up if it wasn't a head shot (or some other similar)
					...
					if damage > 0 then
						...
						elseif myteam == TEAM_HUMAN and otherteam == TEAM_UNDEAD then
							...
							if time >= ent.m_LastWaveStartSpawn + 3 and time >= ent.m_LastGasHeal + 2 then
								local points = damage / ent:GetMaxHealth() * ent:GetZombieClassTable().Points * (1 - self.LastHitPointFraction)
...
function GM:StatusFloater(attacker, victim, dmgpos, amount, statusname)
	if attacker == victim or not attacker then
		return
	end
	if dmgpos == vector_origin then dmgpos = victim:NearestPoint(attacker:EyePos()) end

	local statusIndex = self.StatusFloaters[statusname].index

	net.Start("zs_dmg_status")
		net.WriteUInt(math.ceil(amount), 16)
		net.WriteVector(dmgpos)
		net.WriteUInt(statusIndex, 16)
	net.Send(attacker)
end
...
function GM:KeyRelease(pl, key)
	if key == IN_SPEED then
		if pl:Alive() and pl:Team() == TEAM_UNDEAD then
			pl:CallZombieFunction0("AltRelease")
		end
	end
end
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	attacker.PointQueue = attacker.PointQueue + pl:GetZombieClassTable().Points * self.LastHitPointFraction
	...
	if attacker.BattleCader then
		attacker:SetOomph(attacker:GetOomph() + self.BattleCaderOomphRegen)
	end
...
function GM:ZombieKilledHuman(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	local assister
	if attacker:IsBot() and pl.LastPlayerAttacker and pl.LastPlayerAttacked + 10 > CurTime()
	and pl.LastPlayerAttacker:IsValidZombie() and pl.LastPlayerAttacker ~= pl then
		--the last hitter was a bot, and pl was hit by a different player who is a
		--valid zombie within the last 10 seconds, so we want to give them the brain
		--instead, and mark the player as the assister
		assister = pl.LastPlayerAttacker
		assister:AddBrains(1)
		assister:AddLifeBrainsEaten(1)
		assister:AddZSXP(self.InitialVolunteers[assister:UniqueID()] and xp or math.floor(xp/4))
	...
	if not pl.Gibbed and not suicide then
		...
		pl.DeathClass = self:FindZombieClassByName("Fresh Dead")
	end
...
function GM:DoPlayerDeath(pl, attacker, dmginfo)
	...
	local ct = CurTime()
	local suicide = (attacker == pl or attacker:IsWorld()) and inflictor == attacker
	...
	if plteam == TEAM_UNDEAD then
		...
		if classtable:IsBoss() and not self.ObjectiveMap and pl.BossDeathNotification then
	...
	elseif plteam == TEAM_HUMAN then
		...
		if attacker:IsPlayer() and attacker ~= pl then
			assistpl = gamemode.Call("ZombieKilledHuman", pl, attacker, inflictor, dmginfo, headshot, suicide)
		end
...
--braincell functions
function GM:GetBrainCells( pl )
	return pl.BrainCells
end

function GM:AddBrainCells( pl, amount )
	local newtotal = math.min(self:GetBrainCells(pl) + amount,self.MaxBrainCells:GetInt())
	self:SetBrainCells(pl, newtotal)
end

function GM:SetBrainCells( pl, amount )
	pl.BrainCells = amount
	net.Start("zs_braincells_set")
		net.WriteInt(amount,16)
	net.Send(pl)
end

--wave end healing
function GM:DoWaveEndHeal()
	for _, pl in pairs(player.GetAll()) do
		if pl:Team() ~= TEAM_HUMAN then continue end
		if not pl.RespiteThreshold or pl.RespiteThreshold == 0 then continue end

		local hp = pl:Health()
		local maxhp = pl:GetMaxHealth()
		local hpfrac = hp/maxhp
		local healfrac = math.max(pl.RespiteThreshold - hpfrac, pl.RespiteMin)

		pl:HealPlayer(pl,healfrac * maxhp, 0, true)

		local amt = pl:Health() - hp
		net.Start("zs_waveend_heal")
			net.WriteInt(amt,16)
		net.Send(pl)
	end
end

--wave end zombie burn
function GM:DoWaveBurn()
	for _, pl in pairs(player.GetAll()) do
		if pl:Team() ~= TEAM_UNDEAD then continue end

		pl:GiveStatus("waveburn",-1)
	end
end

--glory functions
function GM:ResetGlory( pl )
	pl.LastGloryScore = 0
end

function GM:CheckGlory( pl )
	if not self.DoGlory:GetBool() then return end

	local points = pl:Frags()

	--check the next glory score to see if you have that many points
	local nextScore = pl.LastGloryScore + self.GloryScore:GetInt()
	if(points >= nextScore) then
		local hp = self.GloryHealth:GetInt()
		--the player should get their glory health
		pl.LastGloryScore = nextScore

		--send net message
		net.Start("zs_glory_heal")
			net.WriteInt(nextScore,16)
			net.WriteInt(hp,16)
		net.Send(pl)

		--heal player
		pl:HealPlayer(pl,hp, 0, true)
	end
end
--upgrade functions
function GM:ResetUpgrades( pl, soft)
	if soft and pl.LastUpgradeGroup and pl.LastUpgradeGroup > UPGRADEGROUP_NONE then
		--player was redeemed. they don't need to repick starting gear.
		pl.LastUpgradeGroup = UPGRADEGROUP_T1_MELEE
	else
		--player probably didn't redeem. let them repick starting gear
		pl.LastUpgradeGroup = UPGRADEGROUP_NONE
	end
	pl.CurrentUpgradeGroup = UPGRADEGROUP_NONE
	pl.CurrentUpgradeItems = {} --3 string signatures
	self:CheckUpgrades(pl, true)
end

function GM:CheckUpgrades( pl, doReportNone)

	--if an upgrade is already in progress, no need to change anything
	if pl.CurrentUpgradeGroup ~= UPGRADEGROUP_NONE then
		return true --there's already an upgrade
	end

	local points = pl:Frags()

	--check the next upgrade group to see if you have that many points
	local nextGroup = pl.LastUpgradeGroup + 1
	if(self.UpgradeGroups[nextGroup] and points >= self.UpgradeGroups[nextGroup].points) then
		--a new upgrade should be generated for this tier and sent to the player
		pl.CurrentUpgradeGroup = nextGroup
		local count = (nextGroup ~= UPGRADEGROUP_T1) and (pl.UpgradeCount or 3) or 3
		pl.CurrentUpgradeItems = self:GenerateUpgradeItems( nextGroup, count )

		--send the upgrade to the player
		net.Start("zs_upgrade_unlocked")
			net.WriteInt(nextGroup,8)
			net.WriteUInt(#pl.CurrentUpgradeItems, 8)
			local k, v
			for k, v in ipairs(pl.CurrentUpgradeItems) do
				net.WriteString(v)
			end
		net.Send(pl)
	else
		--there wasn't an upgrade. should we tell the client?
		if doReportNone then
			net.Start("zs_upgrade_unlocked")
				net.WriteInt(UPGRADEGROUP_NONE,8)
			net.Send(pl)
		end
		return false --there wasnt an upgrade
	end

	return true --there WAS an upgrade
end

function GM:CheckUpgradeOnPointAdd( pl )
	self:CheckUpgrades(pl)
	self:CheckGlory(pl)
end

function GM:GenerateUpgradeItems( upgradeGroup, count )

	local Items = {} --a table of 3 signatures
	local PossibleItems = {} --a table of signatures

	--iterate over the upgrade table and find all same-grouped guns
	--then add them to PossibleItems
	local k, tab
	for k, tab in ipairs(self.Upgrades) do
		if tab.group == upgradeGroup then
			--this item belongs to this group, so add it to possible list
			PossibleItems[#PossibleItems + 1] = tab.signature
		end
	end

	--pick <count> random items, while maintaining proper order
  ...
  for i = #Nums, 2, -1 do
	...
	--remove the ending elements
	while #Nums > count do
		table.remove(Nums) --this removes the final value
	end

	--sort Nums ascending
	table.sort(Nums, function(a,b) return a < b end)

	--add all these items to Items
	for _, i in ipairs(Nums) do
		Items[#Items+1] = PossibleItems[i]
	end

	--return the table of signatures
	return Items
end

function GM:SelectUpgrade( pl, card )
	if pl.CurrentUpgradeGroup == UPGRADEGROUP_NONE then
		return
	elseif pl.CurrentUpgradeGroup == UPGRADEGROUP_T1 then
		if not pl.ActivatedHumanSkills then
			pl:ActivateHumanSkills()
		end
	end

	local itemTab = self.Items[pl.CurrentUpgradeItems[card]]
	if not itemTab then return end

	pl:GiveItem( itemTab )

	pl.LastUpgradeGroup = pl.CurrentUpgradeGroup
	pl.CurrentUpgradeGroup = UPGRADEGROUP_NONE
	pl.CurrentUpgradeItems = {}
end

function GM:DoResupply()
	gamemode.Call("SetNextResupply", CurTime() + self:GetResupplyTime())

	local doDouble = self.CappedInfliction >= self.DoubleResupplyInfliction
	--resupply the humans
	local humans = team.GetPlayers(TEAM_HUMAN)
	for _, pl in pairs(humans) do
		if not pl:IsValid() then continue end

		if pl.MaxStockpiles and pl.MaxStockpiles > 0 then
			if not pl.Stockpiles then pl.Stockpiles = 0 end
			if pl.Stockpiles < pl.MaxStockpiles then
				pl.Stockpiles = pl.Stockpiles + 1
				net.Start("zs_stockpile")
					net.WriteUInt(pl.Stockpiles, 8)
				net.Send(pl)
...
function GM:StartReinforcements()
	local wave = self:GetNextActiveWave()
	self.LastReinforcement = 0

	self:SetNextReinforcement(self.ReinforcementTimes[wave] and (CurTime() + self.ReinforcementTimes[wave][1]) or 0)
end
function GM:DoReinforcements()
	//figure out when the next reinforcement should be, and if we even have one
	local wave = self.GetWave()
	if not self.LastReinforcement then
		self.LastReinforcement = 1
	else
		self.LastReinforcement = self.LastReinforcement + 1
	end
	local nextDuration = self.ReinforcementTimes[wave] and self.ReinforcementTimes[wave][self.LastReinforcement + 1] or nil
	self:SetNextReinforcement(nextDuration and (self:GetNextReinforcement() + nextDuration) or 0)
	self.LastReinforcementTime = CurTime()

	//actually add reinforcements
	self:SetReinforcementCount(self:GetReinforcementCount() + self:GetMiniBossCount())

	//tell people
	net.Start("zs_reinforcements")
		net.WriteUInt(self:GetMiniBossCount(),8)
	net.Broadcast()
end
function GM:ResetReinforcements()
	self.LastReinforcement = nil
	self:SetReinforcementCount(0)
	self:SetNextReinforcement(0)
	self.LastReinforcementTime = 0
end

function GM:FindItem( itemstr )

	--first, see if this string looks like a swep name
	if string.StartWith(itemstr, "weapon_") then
		--this is likely a swep. search the table for it
		for _, item in ipairs(self.Items) do
			if item.swep and item.swep == itemstr then
				return item
			end
		end
	elseif string.StartWith(itemstr, "item_") then
		--this is likely an item. search the table for it
		for _, item in ipairs(self.Items) do
			if item.item and item.item.ClassName == itemstr then
				return item
			end
		end
	end

	--finally, check if this is the signature of an item
	if self.Items[itemstr] then
		return self.Items[itemstr]
	end

	return false
end

function GM:FindItems( itemstr )
	local foundItems = {}

	for _, item in ipairs(self.Items) do
		if string.match(item.signature, itemstr)
			or (item.swep and string.match(item.swep, itemstr))
			or (item.item and string.match(item.item.ClassName, itemstr))
			then
...
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		timer.Remove("RespawnDelayed"..pl:EntIndex())
		...
		elseif pl.DeathClass then --and
			pl:SetZombieClass(pl.DeathClass)
			pl.DeathClass = nil
		elseif self:GetWaveActive() then
			if self:GetReinforcementCount() > 0 and self.LastReinforcementTime and self.LastReinforcementTime + (pl:IsBot() and 5 or 3) < CurTime() then
				//we have a miniboss life, and we didn't VERY RECENTLY get any new ones. so just force them to miniboss
				self:SetReinforcementCount(self:GetReinforcementCount() - 1)
				pl:SetZombieClass(pl:GetSelectedMiniBossClass())

			else
				local cur = pl:GetZombieClass()
				local new = pl:GetSelectedZombieClass()
				new = self:ValidateZombieClass(new, ZTYPE_NORMAL) and new or self.DefaultZombieClass
				new = self:GetBestAvailableZombieClass(new) or new

				if cur ~= new then
					pl:SetZombieClass(new)
				end
			end
		end
		...
		if classtab.Type ~= ZTYPE_NORMAL then
			...
			local lowundead = false --team.NumPlayers(TEAM_UNDEAD) < 4
			--important
		...
		*/
		...
		local hp = classtab:CalcMaxHealth()
		pl:SetHealth(hp)
		pl:SetMaxHealth(hp)
	...
	elseif pl:Team() == TEAM_HUMAN then
		...
		if not self.NoSkills and self:GetWave() > 0 then
			pl:ActivateHumanSkills()
		end
...
function GM:WaveStateChanged(newstate)
	--newstate is bool where true means wave is now active and false means now inactive
	if newstate then --if the wave just started
		--IF THIS IS WAVE 1 (if the last wave was wave 0)
		if self:GetWave() == 0 then
			--create the stupid bullshit sigils
			--gamemode.Call("CreateSigils", true) -- Try creating sigils again. Only really matters if nobody seeded the map yet.

			--turn zombie volunteers into zombies
			self:SetClosestsToZombie()

			--populate the human and zombie teams
			local humans = {}
			for _, pl in pairs(player.GetAll()) do
				if pl:Team() == TEAM_HUMAN and pl:Alive() then
			...
			--start resupply period
			gamemode.Call("SetNextResupply", CurTime() + self:GetResupplyTime())

			--give perks to players who have not yet gotten their perks
			for _, pl in pairs(humans) do
				if pl.PlayerReady and not pl.ActivatedHumanSkills then -- There's a chance they might not be ready to send their desired cart yet.
					pl:ActivateHumanSkills()
				end
			end
		...
		self:StartReinforcements()
	elseif self:GetWave() >= self:GetNumberOfWaves() then --if the last wave just ended
		...
		for _, pl in pairs(player.GetAll()) do
			...
			elseif pl:Team() == TEAM_UNDEAD then
				local didRedeem = pl:CheckRedeem(nil, nil, true)

				if not didRedeem and not pl:Alive() and not pl.Revive then
...
function GM:SpawnBoss(pl, includeNoBossPlayers, includeBosses, silent, bossindex, triggerboss)
	if pl == nil then
		pl = self:GetMVPZombie(includeNoBossPlayers, includeBosses)
	end

	if not pl then return end

	bossindex = bossindex or pl:GetSelectedBossClass()

	if not triggerboss then
		pl.BossDeathNotification = true
...
function GM:SpawnMiniBoss(pl, includeNoBossPlayers, includeBosses, loud)
	if pl == nil then
		pl = self:GetMVPZombie(includeNoBossPlayers, includeBosses)
	end
	...
	local bossindex = pl:GetSelectedMiniBossClass()

	pl:BecomeClass(bossindex, loud)
...
function GM:SpawnBosses(number, includeNoBossPlayers, includeBosses)
	local pls = self:GetMVPZombies(includeNoBossPlayers, includeBosses)

	number = math.min(#pls, number)

	for n = 1, number do
		self:SpawnBoss(pls[n])
	end
end

function GM:SpawnMiniBosses(number, includeNoBossPlayers, includeBosses, loud)
	local pls = self:GetMVPZombies(includeNoBossPlayers, includeBosses)

	number = math.min(#pls, number)

	for n = 1, number do
		self:SpawnMiniBoss(pls[n])
	end
...
function GM:GiveItem(pl, itemTab, noammo)
	pl:GiveItem(itemTab,noammo)
end

net.Receive("zs_buyclass", function(len, sender)
	...
	local classindex = GAMEMODE:GetBestAvailableZombieClass(net.ReadString())
	local classtab = GAMEMODE.ZombieClasses[classindex]

	--double check this is a special zombie
	if not classtab or classtab.Disabled or classtab.Hidden or not classtab.Special then return end

	--check if we are on the right wave
	if GAMEMODE:GetWave() + (GAMEMODE:GetWaveActive() and 1 or 0) < (classtab.Wave or 0) then
		sender:CenterNotify(COLOR_RED, translate.ClientFormat(sender, "class_not_unlocked_will_be_unlocked_x", classtab.Wave))
	else
		local cells = GAMEMODE:GetBrainCells(sender)
		if cells < classtab.SpecialCost then
			sender:CenterNotify(COLOR_RED, "You cannot afford this class. (" .. cells .. "/" .. classtab.SpecialCost .. " Cells)")
		else
			if not GAMEMODE:GetWaveActive() then
				sender:CenterNotify(COLOR_RED, "Please wait for the round to start. (for this patch :P)")
			else
				--unlock a life as that class
				GAMEMODE:SetBrainCells(sender, cells - classtab.SpecialCost)

				--sender:KillSilent()
				sender:BecomeClass(classindex)

				net.Start("zs_special_purchased")
					net.WriteUInt(classindex, 8)
				net.Send(sender)
			end
		end
	end
...
net.Receive("zs_changeclass", function(len, sender)
	...
	local classtype = net.ReadUInt(8)

	if GAMEMODE:ValidateZombieClass(classindex, classtype) then
		if classtype == ZTYPE_NORMAL then
			sender:SetSelectedZombieClass(classindex)
			local force = net.ReadBool()
			if force and GAMEMODE:GetWaveActive() then
				if sender:IsValidLivingZombie() and sender:GetClassType() ~= ZTYPE_BOSS then
					sender:RespawnDelayed()
				elseif sender:IsValidZombie() and not sender:Alive() then
					if not timer.Exists("RespawnDelayed"..sender:EntIndex()) then
						sender:BecomeClass(classindex)
					end
				end
			end
		elseif classtype == ZTYPE_BOSS then
			sender:SetSelectedBossClass(classindex)
		elseif classtype == ZTYPE_MINIBOSS then
			sender:SetSelectedMiniBossClass(classindex)
		end
	else
		print("class" .. classindex .. "did not validate as ztype " .. classtype)
	end
end)
...
net.Receive("zs_nestspec", function(len, sender)
	...
	local doForce = net:ReadBool()
	local nestclass = nest and nest:IsValid() and nest:GetClass() or nil
	if not nestclass then
		if GAMEMODE:GetReinforcementCount() > 0 then
			if ((doForce and not sender:IsBossOrMini()) or not sender:Alive()) then
				GAMEMODE:SpawnMiniBoss(sender)
				GAMEMODE:SetReinforcementCount(GAMEMODE:GetReinforcementCount() - 1)
			end
		else
			return
		end
	...
	if nestclass == "prop_creepernest" then
		if doForce then
			--force the player to respawn at this nest
			if sender:Alive() and not sender:IsBossOrMini() then
				--kill them first
				sender:RespawnDelayed(nest)
			elseif not sender:Alive() then
				--just spawn them now
				sender.ForceDynamicSpawn = nest --if a nest isn't supplied, this will be nil and a normal spawn is used
				sender:UnSpectateAndSpawn()
			end
		elseif nest:IsValid() && not sender:Alive() then
			--if the player is dead, make them spectate the nest
			sender:Spectate(OBS_MODE_CHASE)
			sender:SpectateEntity(nest)
		end
	elseif nestclass == "prop_darknest" then
		if (doForce or not sender:Alive()) and nest:IsValid() then
			GAMEMODE:SpawnMiniBoss(sender)
...
concommand.Add("zs_givecells", function(sender, command, arguments)
	if not sender:IsValid() then return end

	name = string.lower(arguments[1]);
	for _,v in ipairs(player.GetHumans()) do
		if(string.find(string.lower(v:Name()),name,1,true) != nil) then
			GAMEMODE:AddBrainCells(v,tonumber(arguments[2]))
		end
	end

end,nil,nil,FCVAR_CHEAT)

concommand.Add("zs_test", function(sender, command, arguments)
	if not sender:IsValid() then return end
	print("test")
