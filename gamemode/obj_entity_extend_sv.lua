...
function meta:GetDefaultBarricadeHealth()
	...
	return math.Clamp((GAMEMODE.BarricadeHealthMin + mass * GAMEMODE.BarricadeHealthMassFactor + self:GetVolume() * GAMEMODE.BarricadeHealthVolumeFactor) * scalar, GAMEMODE.BarricadeHealthMin, GAMEMODE.BarricadeHealthMax)
end
...
function meta:DamageNails(attacker, inflictor, damage, dmginfo)
	...
	if attacker:IsPlayer() then
		-- :O) --do extra damage to props if you have spawn protection
...
function meta:GiveItem(itemTab, noammo )

	if itemTab.swep then
		--give the player the requested weapon
		local wep
		if self:HasWeapon(itemTab.swep) then
			--drop an empty one and give the player the ammo
			local stored = weapons.Get(itemTab.swep)
			wep = ents.Create("prop_weapon")
			if wep:IsValid() then
			...
			--setting wep to the player's weapon means at the bottom, the
			--player gets 2 clips for the gun
			--and then right here we give the player the amount of bullets that would
			--spawn in the clip
			wep = self:GetWeapon( itemTab.swep )
			if wep.Primary and wep.Primary.Ammo then
				self:GiveAmmo(wep.Primary.ClipSize, string.lower(wep.Primary.Ammo))
			end
		else
			wep = self:Give(itemTab.swep)
			if wep and wep:IsValid() and wep.EmptyWhenPurchased and wep:GetOwner():IsValid() then
				if wep.Primary then
					local primary = wep:ValidPrimaryAmmo()
					if primary then
						self:RemoveAmmo(math.max(0, wep.Primary.DefaultClip - wep.Primary.ClipSize), primary)
					end
				end
				if wep.Secondary then
					local secondary = wep:ValidSecondaryAmmo()
					if secondary then
						self:RemoveAmmo(math.max(0, wep.Secondary.DefaultClip - wep.Secondary.ClipSize), secondary)
					end
				end
			end
		end

		if not noammo and wep and wep:IsValid() and wep:IsWeapon() then
			if wep.Primary and wep.Primary.Ammo then
				local ammo = string.lower(wep.Primary.Ammo)
				if ammo and ammo ~= "none" and GAMEMODE.AmmoCache[ammo] then
					local amount = GAMEMODE.AmmoCache[ammo] * GAMEMODE.SurvivalClips
					self:GiveAmmo(amount, ammo)
				end
			end
		end
	else
		--this is an item, run the give function
		itemTab.item.GiveFunction(self)
	end
end
