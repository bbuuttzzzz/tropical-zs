...
function EFFECT:Render()
	...
	cam_Start3D2D(self.Pos + math_sin(CurTime() + self.Seed) * 30 * delta * right, ang, (delta * 0.12 + 0.045) / 2)
		...
		if amount == flooramount then
			draw_SimpleText(translate.Format("fs_hp_cade_x", amount), "ZS3D2DFont2Big", 0, -21, col, TEXT_ALIGN_CENTER)
		else
			draw_SimpleText(flooramount, "ZS3D2DFont2Big", 0, -21, col, TEXT_ALIGN_RIGHT)
			draw_SimpleText(translate.Format("fs_hp_cade_x", tostring(amount - flooramount):sub(2)), "ZS3D2DFont2", 2, 8, col, TEXT_ALIGN_LEFT)
		end
	cam_End3D2D()
	cam_IgnoreZ(false)
end
