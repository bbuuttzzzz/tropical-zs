...
function ENT:AddDamage(damage, attacker)
	...
	local diff = self:GetDamage() + damage - GAMEMODE.MaxRadiationDamage or 1000
...
function ENT:ApplyHeal(amount)
	local power = amount * GAMEMODE.RadiationHealRate

	local remainder = power - self:GetDamage()

	if remainder >= 0 then
		self:SetDamage(0)
		return remainder / GAMEMODE.RadiationHealRate
	else
...
function ENT:SetDamage(damage)
	self:SetDTFloat(0, math.min(GAMEMODE.MaxRadiationDamage or 1000, damage))
end
