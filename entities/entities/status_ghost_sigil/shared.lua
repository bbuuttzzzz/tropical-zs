...
ENT.GhostModel = Model("models/squad/sf_plates/sf_plate3x3.mdl")
ENT.GhostRotation = Angle(270, 0, 0)
ENT.GhostDrawOffset = Vector(-18.333,-18.333,0)
ENT.GhostEntity = "prop_obj_sigil"
ENT.GhostWeapon = "weapon_zs_sigilseed"
ENT.GhostDistance = 70
ENT.GhostFlatGround = true
ENT.GhostRotateFunction = "Up"
ENT.GhostLimitedNormal = 0.90

function ENT:Initialize()
	self:DrawShadow(false)
	--self:SetMaterial("models/wireframe") --models/shiny
...
function ENT:RecalculateValidity()
	...
	local entity
	local tr = util.TraceLine({start = shootpos, endpos = shootpos + Vector(0,0,-1) * self.GhostDistance, mask = MASK_SOLID_BRUSHONLY, filter = owner})
	...
	ang = Angle(0,0,0)
	self:SetPos(pos + self.GhostDrawOffset)
	self:SetAngles(ang)
