...
function ENT:DrawTranslucent()
	...
	local radius = (700 + math_cos(sat) * 40) * scale
	local whiteradius = (300 + math_sin(sat) * 32) * scale
	local up = self:GetUp()
	local spritepos = self:GetPos() + up
	local spritepos2 = self:WorldSpaceCenter()
	r = (0.15 + colsat) * healthperc
	g = (0.4 + colsat) * healthperc
	b = 1 * healthperc
	...
	local particle2 = emitter:Add(corrupt and "particle/smokesprites_0001" or "sprites/glow04_noz", pos)
	particle2:SetDieTime(math.Rand(1.5, 4))
	particle2:SetPos(self:GetPos() + Vector(math.Rand(-25,25),math.Rand(-25,25),0))
	particle2:SetVelocity(Vector(0, 0, math.Rand(0, 5) * scale))
	particle2:SetStartAlpha(math.Rand(25,50))
	particle2:SetEndAlpha(math.Rand(25,150))
	particle2:SetStartSize(math.Rand(25, 55))
	particle2:SetEndSize(0)
	particle2:SetStartLength(math.Rand(5,150))
	particle2:SetEndLength(math.Rand(0,150))
	particle2:SetRoll(math.Rand(0, 360))
	particle2:SetRollDelta(math.Rand(-1, 1))
	particle2:SetColor(r * 255, g * 255, b * 255)
