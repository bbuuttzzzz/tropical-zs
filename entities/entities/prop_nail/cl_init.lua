		...
		cam.Start3D2D(nearest, ang, 0.1)
			...
			if self:GetMaxRepairs() > 0 or self:GetMaxNailHealth() > 0 then
				...
				if displayowner then
					...
					draw.SimpleText(displayowner, "ZS3D2DUnstyleSmallest", x, y + 10, col, TEXT_ALIGN_LEFT)
					draw.SimpleText(math.floor(nhp) .. "/" .. math.floor(self:GetMaxNailHealth()), "ZS3D2DUnstyleTiny", x - 5, y - 10, col, TEXT_ALIGN_RIGHT)
				end
			end
		cam.End3D2D()
