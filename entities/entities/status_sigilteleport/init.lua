...
function ENT:Think()
	...
	if CurTime() >= self:GetEndTime() then
		if self.BackupPlan then
			owner:DoBackupPlanTeleport(froms)
			self:Remove()
			return true
		end
		if self:GetTargetSigil():IsValid() then
			owner:DoSigilTeleport(self:GetTargetSigil(), froms)
		end

		self:Remove()
	end

	if froms and froms:IsValid() and not froms:IsWeapon() and (owner:GetPos():DistToSqr(froms:GetPos()) > 16384) then
		self:Remove()
	end
