...
function SWEP:GetCone()
	...
	local latespreadmul = 1
	if owner.Crackshot and owner:Crouching() and speedFrac < 0.1 then
		latespreadmul = 1 - ((0.1-speedFrac) * 5)
	end

	return (basecone + conedelta * (self.FixedAccuracy and 0.6 or multiplier) ^ self.ConeRamp) * spreadmul * orphic * latespreadmul
end

function SWEP:GetWalkSpeed()
	local owner = self:GetOwner()
	return (GAMEMODE.PlayerSpeed - (owner.WeaponWeightMul or 1) * self.WeightClass) * (self:GetIronsights() and 0.5 or 1)
end
