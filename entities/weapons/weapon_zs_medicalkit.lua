...
SWEP.Description = "use PRIMARY FIRE to heal others, SECONDARY FIRE to heal yourself. can treat status effects"

SWEP.TranslationName = "wep_medicalkit"
SWEP.TranslationDesc = "wep_d_medicalkit"
SWEP.FakeClip = true --for use with weapon select, writes total ammo as one number instead of 2

SWEP.Slot = SLOT_TOOL
SWEP.SlotPos = SLOTPOS_TOOL
...
function SWEP:PrimaryAttack()
	...
	local healed = owner:HealPlayer(ent, math.min(self:GetCombinedPrimaryAmmo(), self.Heal), 0)
...
function SWEP:SecondaryAttack()
	...
	local multiplier = self.MedicHealMul or 1
	local cooldownmultiplier = self.MedicCooldownMul or 1
	local healed = owner:HealPlayer(owner, math.min(self:GetCombinedPrimaryAmmo(), self.Heal), 0)
...
function SWEP:DrawHUD()
	...
	if 0 < timeleft then
		...
		surface.DrawTexturedRect(x, y, math.min(1, timeleft / math.max(self.Primary.Delay, self.Primary.Delay )) * wid, hei)
