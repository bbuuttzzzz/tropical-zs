...
function SWEP:SecondaryAttack()
	...
	or not trent:IsNailed() and not trent:GetPhysicsObject():IsMoveable() then
		/*
			//just debug to see the reason why we can't nail
			print("1: ",not trent:IsValid())
			print(not util.IsValidPhysicsObject(trent, tr.PhysicsBone))
			print(tr.Fraction == 0)
			print(trent:GetMoveType() ~= MOVETYPE_VPHYSICS and not trent:GetNailFrozen())
			print(trent.NoNails)
			print(trent:IsProjectile())
			print(trent:IsNailed() and (#trent.Nails >= 8 or trent:GetPropsInContraption() >= GAMEMODE.MaxPropsInBarricade))
			print(trent:GetMaxHealth() == 1 and trent:Health() == 0 and not trent.TotalHealth)
			print(trent.PreHoldCollisionGroup and (trent.PreHoldCollisionGroup == COLLISION_GROUP_DEBRIS or trent.PreHoldCollisionGroup == COLLISION_GROUP_DEBRIS_TRIGGER or trent.PreHoldCollisionGroup == COLLISION_GROUP_INTERACTIVE_DEBRIS))
			print(not trent:IsNailed() and not trent:GetPhysicsObject():IsMoveable())
		*/
		return
	end
