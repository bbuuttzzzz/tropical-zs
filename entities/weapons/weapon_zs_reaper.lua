...
SWEP.Description =  "Bonus damage for a while after a kill"

SWEP.TranslationName = "wep_reaper"
SWEP.TranslationDesc = "wep_d_reaper"
...
function SWEP.BulletCallback(attacker, tr)
	...
	if hitent:IsValidLivingZombie() and hitent:Health() <= hitent:GetMaxHealth() * 0.04 and gamemode.Call("PlayerShouldTakeDamage", hitent, attacker) then
		if SERVER then
			hitent:SetWasHitInHead()
		end
		--was DMG_DIRECT. changed so it doesn't break spawn protection. shouldn't break anything
		hitent:TakeSpecialDamage(hitent:Health(), DMG_BULLET, attacker, attacker:GetActiveWeapon(), tr.HitPos)
