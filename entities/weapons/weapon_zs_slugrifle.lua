...
SWEP.Description = "Hitting a headshot magically chambers another round."

SWEP.TranslationName = "wep_slugrifle"
SWEP.TranslationDesc = "wep_d_slugrifle"
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
	if tr.HitGroup == HITGROUP_HEAD then
		--this is the headshot instakill passive which we don't want
		--[[
		local ent = tr.Entity
		if ent:IsValidLivingZombie() and ent:GetZombieClassTable():IsBoss() then
			return
		end
		...
		if gamemode.Call("PlayerShouldTakeDamage", ent, attacker) then
			...
			dmginfo:SetDamage(dmginfo:GetDamage() + ent:GetMaxHealth() * 0.55)
		end
		]]

		local wept = attacker:GetActiveWeapon()

		local clip = wept:Clip1()

		local ammoLeft =  attacker:GetAmmoCount(wept:GetPrimaryAmmoType())

		if clip < wept.Primary.ClipSize and ammoLeft >= 1 then
			wept:SetClip1(clip + 1)
			if SERVER then
				attacker:RemoveAmmo(1,wept:GetPrimaryAmmoType())
			end
		end
