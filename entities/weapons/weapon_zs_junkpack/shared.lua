...
SWEP.JunkModels = {
	...
	Model("models/squad/sf_bars/sf_bar1.mdl"),
	Model("models/gibs/hgibs.mdl")
}

SWEP.JunkModelsHoarder = {
	Model("models/props_wasteland/controlroom_filecabinet002a.mdl"),
	Model("models/props_c17/bench01a.mdl"),
	Model("models/props_c17/oildrum001.mdl"),
	Model("models/props_wasteland/controlroom_chair001a.mdl"),
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if ent:IsValid() then
			...
			ent:SetAngles(ang)
			if self:GetOwner().Hoarder then
				ent:SetModel(self.JunkModelsHoarder[math.random(#self.JunkModelsHoarder)])
			else
...
function SWEP:Think()
	...
	if SERVER and self:GetPrimaryAmmoCount() <= 0 then
