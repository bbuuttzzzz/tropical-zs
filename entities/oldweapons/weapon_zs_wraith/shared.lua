...
function SWEP:Swung()
	...
	GAMEMODE:SetPlayerSpeed(self:GetOwner(), self.oldwalkspeed, self.oldrunspeed)

	self.BaseClass.Swung(self)
end

function SWEP:StartSwinging()
	//self:GetOwner():SetMoveType(MOVETYPE_FLYGRAVITY)
	self.oldwalkspeed = self:GetOwner():GetWalkSpeed()
	self.oldrunspeed = self:GetOwner():GetRunSpeed()
	GAMEMODE:SetPlayerSpeed(self:GetOwner(), 20, 20)
